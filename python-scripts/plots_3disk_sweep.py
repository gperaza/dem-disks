...
def get_tilts():
    tilts = []
    directoryList = [d for d in os.listdir(".") if "tilt" in d and os.path.isdir(d)]
    for d in directoryList:
        dList = d.split(".")
        index = dList.index("tilt")
        tilt = dList[index+1]+"."+dList[index+2]
        if not tilt in tilts:
            tilts.append(tilt)
    tilts.sort()
    return tilts
...
def get_acs():
    acs = []
    directoryList = [d for d in os.listdir(".") if "ac" in d and os.path.isdir(d)]
    for d in directoryList:
        dList = d.split(".")
...
def plot_angle_time_sweep():
    ...
    directoryList.sort()
    for d in directoryList:
        p3d.plot_angle_time(d, plotRelax=True)
...
def plot_vel_tilt_for_acs():
    ...
    for ac in acList:
        ...
        axLinks.set_ylabel("State probability")
        p3d.plot_vel_tilt_for_ac(ac, ax=axAll)
